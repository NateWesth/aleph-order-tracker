...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Fetched open PO quantities for ${poQtyMap.size} active SKUs and open PO vendors for ${poVendorMap.size} active SKUs`,
    );
    ...
    const unresolvedAfterBills = new Set([...activeSkus].filter((sku) => !billMap.get(sku)?.vendorName));
    const latestPurchaseOrderVendors =
      unresolvedAfterBills.size > 0
    ...
    for (const sku of activeSkus) {
      ...
      result[sku] = {
        ...
        vendorName:
          bill?.vendorName || latestPoVendor?.vendorName || openPoVendor?.vendorName || item?.vendorName || "",
...
async function fetchRelevantItemStock(
  accessToken: string,
  orgId: string,
...
async function fetchBillDetail(accessToken: string, orgId: string, billId: string) {
  const data = await fetchZohoPage(accessToken, `${ZOHO_API_URL}/books/v3/bills/${billId}?organization_id=${orgId}`);
  return data.bill || {};
}
...
async function fetchLatestPurchaseOrderVendors(accessToken: string, orgId: string, activeSkus: Set<string>) {
  ...
  console.log(
    `Inspected ${inspectedDetails} recent Zoho purchase orders and resolved latest suppliers for ${latestVendorMap.size} active SKUs`,
  );
...
function isZohoThrottle(message: string): boolean {
  ...
  return (
    m.includes("maximum number of in process requests") || m.includes("too many requests") || m.includes("rate limit")
  );
}
