...
export default function FulfillmentPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [ordersRes, deliveryHistoryRes, profilesRes, settingsRes, cacheRes, statesRes, eventsRes] = await Promise.all([
        supabase
          .from("orders")
        ...
        supabase.from("po_collection_state").select("purchase_order_id, purchase_order_number, vendor_id, vendor_name, assigned_to, status, scheduled_for, notes, completed_at, last_seen_at"),
        supabase
      ...
      const readyDeliveries = activeBase
        .map(decorateOrder)
        .filter((order) => order.fulfillment_method === "delivery" && order.items.some((item) => readyUnits(item) > 0) && order.fulfillment_status !== "completed");
  ...
  const collectionQueue = useMemo<CollectionPOView[]>(() => {
    ...
    return purchaseOrders
      .map((po) => {
        ...
        const collectedUnits = linesView.reduce((sum, line) => sum + Math.min(line.collected, Math.max(0, Number(line.outstanding || 0))), 0);
  ...
  const completeDelivery = async (order: FulfillmentOrder) => {
    ...
    try {
      ...
      toast({
        ...
        description: fullyDone ? `${order.order_number} moved to Delivery History.` : `Ready quantities completed. Remaining items stay active.`,
      });
      await fetchData();
    } catch (error: any) {
  ...
  const collectionFiltered = useMemo(() => {
    ...
    return collectionQueue.filter((po) => {
      if (!needle) return true;
      return [po.purchaseOrderNumber, po.vendorName, ...po.linesView.map((line) => `${line.sku || ""} ${line.name} ${line.description || ""}`)]
        .join(" ")
        .toLowerCase()
        .includes(needle);
    });
