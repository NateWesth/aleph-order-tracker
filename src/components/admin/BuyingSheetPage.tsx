...
export default function BuyingSheetPage() {
  ...
  const [snapshotData, setSnapshotData] = useState<{ date: string; rows: { sku: string; toOrder: number }[] } | null>(
    null,
  );
  ...
  useLiveData(
    ["orders", "order_items", "order_purchase_orders"],
    () => {
      fetchLocalData();
      fetchLastPurchaseDates();
    },
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "p" && !e.metaKey && !e.ctrlKey)
        setViewDensity((v) => {
          const next = v === "compact" ? "comfortable" : "compact";
          saveDensity(next);
          return next;
        });
    };
  ...
  const fetchDemandHistory = async () => {
    try {
      ...
      new Set([...lastMap.keys(), ...prevMap.keys()]).forEach((sku) => {
    ...
    try {
      ...
      (data || []).forEach((item) => {
        ...
        const days = Math.max(
          0,
          Math.round(
            (new Date(item.completed_at).getTime() - new Date(item.created_at).getTime()) / (1000 * 60 * 60 * 24),
          ),
        );
  ...
  const fetchZohoData = async (): Promise<ZohoStockData | null> => {
    ...
    try {
      ...
      toast({
        title: "Zoho Data",
        description: result.error || result.message || "Could not fetch supplier and stock data from Zoho",
        variant: "destructive",
      });
      return null;
    } catch (error) {
  ...
  const fetchSuggestedRestock = async (activeSkus: Set<string>) => {
    try {
      ...
      const skuMonthMap = new Map<
        string,
        { itemName: string; months: Map<string, number>; totalOrders: number; lastDate: string }
      >();
      for (const item of completedItems) {
        ...
        if (existing) {
          const m = existing.months.get(monthKey) || 0;
          existing.months.set(monthKey, m + (item.quantity || 1));
  ...
  const getDemandTrend = (
    sku: string,
  ): { trend: "up" | "down" | "stable" | "new"; lastMonth: number; prevMonth: number } => {
  ...
  const fetchSupplierReliability = async () => {
    try {
      ...
      console.error("Failed to fetch supplier reliability:", err);
    }
  };
  ...
  const fetchWeeklyHistory = async () => {
    try {
      ...
      const lastMap = new Map<string, number>();
      (thisRes.data || []).forEach((i) => {
        const sku = (i.code || "").toUpperCase();
        thisMap.set(sku, (thisMap.get(sku) || 0) + (i.quantity || 1));
      });
      (lastRes.data || []).forEach((i) => {
        const sku = (i.code || "").toUpperCase();
        lastMap.set(sku, (lastMap.get(sku) || 0) + (i.quantity || 1));
  ...
  const fetchCostHistory = async () => {
    try {
      ...
      items.forEach((i) => {
        orderItemCounts.set(i.order_id, (orderItemCounts.get(i.order_id) || 0) + (i.quantity || 1));
      });
      items.forEach((i) => {
        ...
        if (orderTotal && itemCount > 0) {
          ...
          const arr = skuCosts.get(sku) || [];
          arr.push(unitCost);
          skuCosts.set(sku, arr);
        }
      });
      const map = new Map<string, number>();
      skuCosts.forEach((costs, sku) => {
        map.set(sku, Math.round((costs.reduce((a, b) => a + b, 0) / costs.length) * 100) / 100);
      });
      setCostHistory(map);
    } catch (err) {
  ...
  const computeAnalyticalFields = (
    sku: string,
    toOrder: number,
    daysWaiting: number,
    avgLeadTimeDays: number | null,
    orders: { urgency?: string }[],
    recommendedOrderQty: number,
    billUnitCost: number | null = null,
    costSource: "bill" | "item" | null = null,
  ) => {
    // Age escalation
    const leadRef = avgLeadTimeDays || 14;
    const ageRatio = daysWaiting / leadRef;
    const ageEscalation: "green" | "yellow" | "orange" | "red" =
      ageRatio > 2 ? "red" : ageRatio > 1.5 ? "orange" : ageRatio > 1 ? "yellow" : "green";
    ...
    return {
      ...
      estimatedCost,
      unitCost,
      costSource: billUnitCost !== null ? (costSource ?? "bill") : null,
      adjustedOrderQty,
      abcClass: "C" as "A" | "B" | "C",
    };
  };
  ...
  const fetchLocalData = async () => {
    ...
    try {
      const { data: orderItems, error: itemsError } = await supabase
        .from("order_items")
        .select("id, name, code, quantity, qty_on_po, progress_stage, order_id, created_at")
      ...
      const [companiesRes, suppliersRes] = await Promise.all([
        ...
        allSupplierIds.length > 0
          ? supabase.from("suppliers").select("id, name, email").in("id", allSupplierIds)
          : { data: [] },
      ]);
      ...
      for (const item of orderItems) {
        ...
        if (poSupplier) {
          ...
          const s = suppliersMap.get(order.supplier_id);
          supplierName = s?.name || "Unknown";
          supplierId = order.supplier_id;
          supplierEmail = s?.email || undefined;
        }
        ...
        if (existing) {
          ...
          skuMap.set(sku, {
            ...
            orders: [{ orderNumber: order?.order_number || "—", customerName, quantity: outstanding, urgency }],
            supplierName,
            supplierId,
            supplierEmail,
      ...
      const buyingRows: BuyingSheetRow[] = Array.from(skuMap.values()).map((entry) => {
        ...
        const coveragePercent =
          entry.totalNeeded > 0 ? Math.min(100, Math.round((covered / entry.totalNeeded) * 100)) : 100;
        ...
        const analytical = computeAnalyticalFields(
          entry.sku,
          toOrder,
          daysWaiting,
          avgLeadTimeDays,
        ...
        return {
          ...
          supplierName,
          supplierEmail,
          stockOnHand: z.stockOnHand,
          onPurchaseOrder: z.onPurchaseOrder,
          toOrder,
          daysWaiting,
          priorityScore,
          coveragePercent,
      ...
      if (zoho) {
        setRows((prev) =>
          prev.map((row) => {
            ...
            const covered = z.stockOnHand + z.onPurchaseOrder;
            const coveragePercent =
              row.totalNeeded > 0 ? Math.min(100, Math.round((covered / row.totalNeeded) * 100)) : 100;
            ...
            return {
              ...
              safetyStock,
              recommendedOrderQty,
              lastPurchasedDate: z.lastPurchasedDate || row.lastPurchasedDate,
              ...analytical,
            };
          }),
        );
      ...
      setRows((prev) =>
        prev.map((row) => {
          ...
          return {
            ...row,
            supplierName,
            supplierEmail: z.vendorEmail || row.supplierEmail,
            stockOnHand: z.stockOnHand,
            onPurchaseOrder: z.onPurchaseOrder,
            toOrder,
            coveragePercent,
  ...
  const saveSnapshot = () => {
    const snapshot = {
      ...
      rows: sortedRows.map((r) => ({ sku: r.sku, toOrder: r.toOrder })),
    };
    ...
    try {
      ...
      if (orderItemIds.length > 0) {
        const { error } = await supabase
          .from("order_items")
          .update({
            stock_status: "ordered" as any,
            notes: `Marked ordered from buying sheet on ${new Date().toLocaleDateString()}`,
          })
          .in("id", orderItemIds);
        if (error) throw error;
        // Track recently ordered
        const newRecent: RecentlyOrderedItem[] = targetRows.map((r) => ({
          sku: r.sku,
          itemName: r.itemName,
          quantity: r.toOrder,
  ...
  const generateEmailDraft = (supplierName: string) => {
    ...
    const itemLines = items
      .map((r) => {
        const recQty =
          r.recommendedOrderQty > r.toOrder
            ? `${r.toOrder} (+ ${r.safetyStock} safety stock = ${r.recommendedOrderQty})`
            : `${r.toOrder}`;
    ...
    setEmailDraftSubject(
      `Purchase Order — ${supplierName} — ${new Date().toLocaleDateString()} (${items.length} items${urgentItems.length > 0 ? ", URGENT" : ""})`,
    );
  ...
  const handleBatchEmailAllSuppliers = () => {
    ...
    if (supplierGroups.size === 0) {
      toast({ title: "Nothing to order", variant: "destructive" });
      return;
    }
  ...
  const handleCopySelectedPOLines = () => {
    ...
    navigator.clipboard.writeText(
      [
        "SKU\tItem Name\tQty to Order\tSupplier",
        ...target.map((r) => `${r.sku}\t${r.itemName}\t${r.toOrder}\t${r.supplierName}`),
      ].join("\n"),
    );
  ...
  const handleCopySupplierEmails = () => {
    ...
    if (emails.size === 0) {
      toast({ title: "No emails", description: "No supplier emails found", variant: "destructive" });
      return;
    }
  ...
  const handlePrint = () => {
    ...
    for (const [supplier, items] of groups.entries()) {
      ...
      for (const item of items)
        html += `<tr><td style="font-family:monospace;font-size:10px">${item.sku}</td><td>${item.itemName}</td><td class="text-right">${item.totalNeeded}</td><td class="text-right">${item.stockOnHand}</td><td class="text-right">${item.onPurchaseOrder}</td><td class="text-right ${item.toOrder > 0 ? "urgent" : ""}">${item.toOrder}</td><td style="font-size:10px">${item.orders.map((o) => `${o.orderNumber} (${o.customerName})`).join(", ")}</td><td style="font-style:italic;color:#666;font-size:10px">${notes[item.sku] || ""}</td></tr>`;
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = target.map((r) => [
      ...
      r.coveragePercent,
      r.daysWaiting,
      r.stockoutRiskDays ?? "N/A",
      r.dailyBurnRate.toFixed(1),
      r.demandVariability,
      r.distinctCustomers,
      r.priorityScore,
      r.supplierName,
      r.avgLeadTimeDays ?? "N/A",
      r.demandTrend,
      r.seasonalPattern || "N/A",
      notes[r.sku] || "",
  ...
  const uniqueSuppliers = useMemo(() => {
    ...
    rows.forEach((r) => {
      if (r.supplierId) {
        const e = suppliers.get(r.supplierId);
        if (e) {
          e.count++;
          e.totalToOrder += r.toOrder;
        } else
          suppliers.set(r.supplierId, {
            name: r.supplierName,
            count: 1,
            totalToOrder: r.toOrder,
  ...
  const supplierConcentration = useMemo(() => {
    const map = new Map<string, number>();
    rows.forEach((r) =>
      map.set(r.supplierName || "No Supplier", (map.get(r.supplierName || "No Supplier") || 0) + r.toOrder),
    );
  ...
  const priorityCounts = useMemo(() => {
    ...
    rows.forEach((r) => {
      c[getPriorityLevel(r.priorityScore)]++;
    });
    return c;
  }, [rows]);
  ...
  const filteredRows = useMemo(
    () =>
      rows.filter((row) => {
        ...
        return (
          matchesSearch &&
          (supplierFilter === "all" || row.supplierId === supplierFilter) &&
          (!showOnlyNeedOrder || row.toOrder > 0) &&
          (priorityFilter === "all" || getPriorityLevel(row.priorityScore) === priorityFilter)
        );
      }),
  ...
  const multiSupplierSkus = useMemo(() => {
    ...
    skuSuppliers.forEach((suppliers, sku) => {
      if (suppliers.size > 1) multi.add(sku);
    });
    return multi;
  }, [rows]);
  ...
  const topSupplierChips = useMemo(() => {
    ...
    return Array.from(map.entries())
      .sort((a, b) => b[1].toOrder - a[1].toOrder)
      .slice(0, 8);
  }, [rows]);
  ...
  const groupedRows = useMemo(() => {
    ...
    for (const row of sortedRows)
      groups.set(row.supplierName || "No Supplier", [...(groups.get(row.supplierName || "No Supplier") || []), row]);
    return Array.from(groups.entries()).sort(
      (a, b) => b[1].reduce((s, r) => s + r.priorityScore, 0) - a[1].reduce((s, r) => s + r.priorityScore, 0),
    );
  ...
  const totalRecommendedQty = useMemo(
    () => filteredRows.reduce((s, r) => s + r.recommendedOrderQty, 0),
    [filteredRows],
  );
  const totalSafetyBuffer = useMemo(() => filteredRows.reduce((s, r) => s + r.safetyStock, 0), [filteredRows]);

  // ── Inline Helpers ────────────────────────────────────────────────────
  const toggleSelect = (sku: string, shiftKey = false) =>
  ...
  const toggleSelectAll = () =>
    setSelectedSkus(selectedSkus.size === sortedRows.length ? new Set() : new Set(sortedRows.map((r) => r.sku)));
  const selectAllNeedingOrder = () =>
    setSelectedSkus(new Set(sortedRows.filter((r) => r.toOrder > 0).map((r) => r.sku)));
  const fillRecommendedForSelected = () => {
    ...
    setAdjustedQtys((prev) => {
      const next = { ...prev };
      target.forEach((r) => {
        next[r.sku] = r.recommendedOrderQty;
      });
      return next;
    });
    toast({
      title: "Filled",
      description: `Recommended qty applied to ${target.length} item${target.length !== 1 ? "s" : ""}`,
    });
  };
  ...
  const togglePin = (sku: string) => {
    ...
    setPinnedSkus(updated);
    savePinned(updated);
    toast({
      title: pinnedSkus.includes(sku) ? "Unpinned" : "Pinned",
      description: `${sku} ${pinnedSkus.includes(sku) ? "removed from" : "pinned to"} top`,
    });
  };
  ...
  const CoverageBar = ({ percent }: { percent: number }) => (
    <div className="flex items-center gap-2 min-w-[80px]">
      <Progress
        value={percent}
        className={`h-1.5 flex-1 ${percent >= 100 ? "[&>div]:bg-emerald-500" : percent >= 50 ? "[&>div]:bg-amber-500" : "[&>div]:bg-destructive"}`}
      />
  ...
  const DemandTrendIcon = ({
    ...
    <Tooltip>
      ...
      <TooltipContent>
        <p className="text-xs">
          {trend === "up"
            ? `↑ Rising (${prevMonth}→${lastMonth})`
            : trend === "down"
              ? `↓ Falling (${prevMonth}→${lastMonth})`
              : trend === "new"
                ? "New item"
                : `Stable (${lastMonth}/mo)`}
        </p>
    ...
    if (days <= 7)
      return (
        <Tooltip>
          ...
          <TooltipContent>
            <p className="text-xs">⚠️ Stock runs out in ~{days} days</p>
          </TooltipContent>
        </Tooltip>
      );
    if (days <= 14)
      return (
        <Tooltip>
          <TooltipTrigger asChild>
            <Badge className="text-xs gap-1 bg-orange-500 text-white">
              <Timer className="h-3 w-3" />
              {days}d
            </Badge>
          </TooltipTrigger>
          <TooltipContent>
            <p className="text-xs">Stock runs out in ~{days} days</p>
          </TooltipContent>
        </Tooltip>
      );
          ...
          <div>
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between">
                ...
                <Badge
                  variant={row.abcClass === "A" ? "destructive" : row.abcClass === "B" ? "secondary" : "outline"}
                  className="text-[10px]"
                >
                  {row.abcClass}
                </Badge>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{row.lastMonthQty}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Previous:</span>
                <span className="font-medium">{row.prevMonthQty}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Forecast Next:</span>
                <span className="font-bold text-primary">{row.forecastNextMonth}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Daily Burn Rate:</span>
                <span className="font-medium">{row.dailyBurnRate.toFixed(1)}/day</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Weekly Velocity:</span>
                <span className="font-medium">
                  {row.velocityScore}/wk{" "}
                  {row.weeklyTrend !== 0 && (
                    <span className={row.weeklyTrend > 0 ? "text-emerald-600" : "text-destructive"}>
                      ({row.weeklyTrend > 0 ? "+" : ""}
                      {row.weeklyTrend}%)
                    </span>
                  )}
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">Demand Pattern:</span>
                <span
                  className={`font-medium ${row.demandVariability === "erratic" ? "text-destructive" : row.demandVariability === "moderate" ? "text-orange-500" : "text-emerald-500"}`}
                >
                  {row.demandVariability}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Reorder Point:</span>
                <span className="font-medium">{row.reorderPoint}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Safety Stock:</span>
                <span className="font-medium">{row.safetyStock > 0 ? `+${row.safetyStock} buffer` : "—"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Recommended Qty:</span>
                <span className="font-bold text-primary">{row.recommendedOrderQty}</span>
              </div>
              {row.unitCost !== null && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      ({row.costSource === "bill" ? "vendor bill" : "estimated"})
                    </span>
                  </span>
                </div>
              )}
              {row.estimatedCost !== null && (
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">R{row.estimatedCost.toLocaleString()}</span>
                </div>
              )}
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{row.distinctCustomers}</span>
              </div>
              {row.supplierReliability !== null && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Supplier Reliability:</span>
                  <span
                    className={`font-medium ${row.supplierReliability >= 80 ? "text-emerald-600" : row.supplierReliability >= 60 ? "text-orange-500" : "text-destructive"}`}
                  >
              ...
              {row.avgLeadTimeDays !== null && (
                <div className="flex justify-between">
                  ...
                  <span className={`font-medium ${row.daysWaiting > row.avgLeadTimeDays ? "text-destructive" : ""}`}>
                    {row.avgLeadTimeDays}d {row.daysWaiting > row.avgLeadTimeDays ? "(overdue!)" : ""}
                  </span>
                </div>
              )}
              {row.seasonalPattern && (
              ...
              {row.lastPurchasedDate && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{new Date(row.lastPurchasedDate).toLocaleDateString()}</span>
                </div>
              )}
              {row.conflictingUrgency && (
                <div className="p-1.5 rounded bg-destructive/10 border border-destructive/20 text-[10px] text-destructive font-medium">
                  ⚠️ Mixed urgency levels across orders
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 p-1.5 rounded bg-background/60 border border-border">
                ...
                <Input
                  type="number"
                  min={0}
                  value={adjustedQtys[row.sku] ?? row.recommendedOrderQty}
                  onChange={(e) =>
                    setAdjustedQtys((prev) => ({ ...prev, [row.sku]: Math.max(0, parseInt(e.target.value) || 0) }))
                  }
              ...
              <Button
                ...
                size="sm"
                className="w-full justify-start gap-2 text-xs"
                onClick={() => generateEmailDraft(row.supplierName)}
              >
                <Send className="h-3 w-3" />
                Draft Email to {row.supplierName}
              </Button>
              {row.supplierEmail && (
                <Button
                  variant="outline"
  ...
  const renderRow = (row: BuyingSheetRow) => {
    ...
    return (
      <Fragment key={row.sku}>
        <TableRow
          className={`cursor-pointer transition-colors ${getWaitHeatColor(row.daysWaiting)} ${row.toOrder > 0 ? "" : "opacity-60"} ${selectedSkus.has(row.sku) ? "bg-primary/5" : ""} ${row.hasUrgent ? "border-l-2 border-l-destructive" : ""} ${isExpanded ? "bg-muted/20" : ""} ${isPinned ? "border-l-2 border-l-primary bg-primary/[0.02]" : ""}`}
        >
          <TableCell className={`w-8 ${densityPy}`} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-0.5">
              <span
                onClick={(e) => {
                  e.stopPropagation();
                  toggleSelect(row.sku, e.shiftKey);
                }}
                className="inline-flex"
              >
                <Checkbox checked={selectedSkus.has(row.sku)} />
              </span>
              <button
                onClick={() => togglePin(row.sku)}
                className={`p-0.5 rounded hover:bg-muted transition-colors ${isPinned ? "text-primary" : "text-muted-foreground/20 hover:text-muted-foreground"}`}
              >
                {isPinned ? <Pin className="h-3 w-3" /> : <PinOff className="h-3 w-3" />}
              </button>
            </div>
          </TableCell>
          <TableCell className={densityPy} onClick={() => toggleExpand(row.sku)}>
            <div className="flex items-center gap-1">
              {isExpanded ? (
          ...
          <TableCell
            className={`font-mono text-xs text-muted-foreground ${densityPy}`}
          ...
          <TableCell className={`font-medium max-w-[180px] ${densityPy}`} onClick={() => toggleExpand(row.sku)}>
            <div className="flex items-center gap-1 min-w-0">
              ...
              <span
                className={`text-[9px] font-bold px-1 rounded shrink-0 ${row.abcClass === "A" ? "bg-destructive/15 text-destructive" : row.abcClass === "B" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
              >
                {row.abcClass}
              </span>
              {row.conflictingUrgency && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs">⚠️ Mixed urgency levels across orders</p>
                  </TooltipContent>
                </Tooltip>
              )}
            </div>
          </TableCell>
          <TableCell className={`text-right font-semibold ${densityPy}`} onClick={() => toggleExpand(row.sku)}>
            {row.totalNeeded}
          </TableCell>
          ...
          <TableCell className={`text-right ${densityPy}`} onClick={() => toggleExpand(row.sku)}>
            <div className="flex items-center justify-end gap-1">
              ...
              {(() => {
                ...
                return (
                  <span
                    className={`text-[10px] font-medium ${diff.diff > 0 ? "text-destructive" : "text-emerald-600"}`}
                  >
                    {diff.diff > 0 ? "+" : ""}
                    {diff.diff}
                  </span>
          ...
          <TableCell className={`text-center ${densityPy}`} onClick={() => toggleExpand(row.sku)}>
            <div className="flex items-center justify-center gap-1">
              <span
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${row.ageEscalation === "red" ? "bg-destructive" : row.ageEscalation === "orange" ? "bg-orange-500" : row.ageEscalation === "yellow" ? "bg-amber-400" : "bg-emerald-500"}`}
              />
              <span
                className={`text-sm font-medium ${row.daysWaiting > 7 ? "text-destructive" : row.daysWaiting > 3 ? "text-orange-500" : "text-muted-foreground"}`}
              >
                {row.daysWaiting}d
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {isMultiSupplier && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs">⚠️ This item appears with multiple suppliers</p>
                  </TooltipContent>
                </Tooltip>
              )}
              {row.avgLeadTimeDays !== null && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs">Avg lead time: {row.avgLeadTimeDays} days</p>
                  </TooltipContent>
                </Tooltip>
              )}
              {row.seasonalPattern === "peak" && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs">🔥 Peak season</p>
                  </TooltipContent>
                </Tooltip>
              )}
              {row.seasonalPattern === "low" && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs">❄️ Low season</p>
                  </TooltipContent>
                </Tooltip>
              )}
            </div>
            ...
            <div className="flex items-center gap-0.5">
              ...
              >
                <PopoverTrigger asChild>
                  <button
                    onClick={() => {
                      setEditingNote(row.sku);
                      setNoteText(notes[row.sku] || "");
                    }}
                ...
                <PopoverContent className="w-64 p-3" side="left">
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-1 justify-end">
                      ...
                      <Button size="sm" onClick={() => handleSaveNote(row.sku)}>
                        Save
                      </Button>
                    </div>
                  </div>
  ...
  return (
    <TooltipProvider>
      <div
        className={`procurement-page app-no-x-scroll min-w-0 w-full max-w-full space-y-5 overflow-x-hidden ${isFullscreen ? "fixed inset-0 z-50 bg-background p-4 overflow-y-auto overflow-x-hidden" : ""}`}
        ref={printRef}
      >
        {/* ── Sticky Header ── */}
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          const next: ViewDensity = viewDensity === "compact" ? "comfortable" : "compact";
                          setViewDensity(next);
                          saveDensity(next);
                        }}
                        className="h-9 w-9"
                    ...
                    <TooltipContent>
                      <p className="text-xs">{viewDensity === "compact" ? "Comfortable" : "Compact"} view</p>
                    </TooltipContent>
                  </Tooltip>
                  <Button variant="outline" size="sm" onClick={handleExportCSV} className="h-9 gap-1.5">
                    <Download className="h-4 w-4" />
                    {selectedSkus.size > 0 ? `Export (${selectedSkus.size})` : "Export CSV"}
                  </Button>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      ...
                      <DropdownMenuItem onClick={handleExportBySupplier}>
                      ...
                      <DropdownMenuItem onClick={saveSnapshot}>
                        ...
                        Save snapshot{snapshotSaved && " ✓"}
                      </DropdownMenuItem>
                      {snapshotData && (
                        <DropdownMenuItem onClick={() => setShowSnapshot(!showSnapshot)}>
                          <History className="h-4 w-4 mr-2" />
                          {showSnapshot ? "Hide comparison" : "Compare to snapshot"}
                        </DropdownMenuItem>
                      )}
                    </DropdownMenuContent>
                  </DropdownMenu>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as ViewMode)} className="shrink-0">
                  <TabsList className="h-8 bg-muted/50">
                    <TabsTrigger value="quick" className="text-xs gap-1 h-7 px-2.5">
                      <Zap className="h-3 w-3" />
                      Queue
                    </TabsTrigger>
                    <TabsTrigger value="suppliers" className="text-xs gap-1 h-7 px-2.5">
                ...
                <div className="relative flex-1 min-w-0">
                  ...
                  <Input
                    placeholder="Search SKU, item, order, customer..."
                    value={search}
                ...
                >
                  ...
                  {showOnlyNeedOrder ? "Needs Order" : "Show All"}
                </Button>
              </div>
            </div>
          </div>
        ...
        {showSnapshot && snapshotData && (
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border text-xs">
            ...
            <span className="text-muted-foreground">
              Comparing to <strong>{new Date(snapshotData.date).toLocaleString()}</strong> ({snapshotData.rows.length}{" "}
              items)
            </span>
            <Button variant="ghost" size="sm" className="ml-auto h-6 text-xs" onClick={() => setShowSnapshot(false)}>
              Hide
            </Button>
          </div>
        )}
        ...
        {topSupplierChips.length > 1 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded-full text-xs transition-colors border ${supplierFilter === "all" ? "bg-primary text-primary-foreground border-primary" : "bg-muted/50 text-muted-foreground border-border hover:bg-muted"}`}
            >
              All
            </button>
            {topSupplierChips.map(([name, info]) => (
              <button
                key={name}
                onClick={() => {
                  if (info.id) setSupplierFilter(info.id);
                }}
                className={`px-2 py-0.5 rounded-full text-xs transition-colors border ${info.id && supplierFilter === info.id ? "bg-primary text-primary-foreground border-primary" : "bg-muted/50 text-muted-foreground border-border hover:bg-muted"}`}
              >
              ...
              <CollapsibleTrigger asChild>
                <button className="w-full flex items-center justify-between p-3 hover:bg-emerald-500/10 transition-colors rounded-t-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm text-foreground">Ordered Today</span>
        ...
        <Tabs value={priorityFilter} onValueChange={(v) => setPriorityFilter(v as PriorityFilter)}>
          <TabsList className="bg-muted/50 h-8">
            <TabsTrigger value="all" className="gap-1 text-xs h-7">
            ...
            <TabsTrigger value="critical" className="gap-1 text-xs h-7">
              <Flame className="h-3 w-3" />
              Critical{" "}
              <Badge
                variant={priorityCounts.critical > 0 ? "destructive" : "outline"}
                className="text-[10px] px-1 py-0 ml-0.5"
              >
                {priorityCounts.critical}
              </Badge>
            </TabsTrigger>
            <TabsTrigger value="high" className="gap-1 text-xs h-7">
              <AlertTriangle className="h-3 w-3" />
              High{" "}
              <Badge className="text-[10px] px-1 py-0 ml-0.5 bg-orange-500 text-white">{priorityCounts.high}</Badge>
            </TabsTrigger>
            <TabsTrigger value="medium" className="gap-1 text-xs h-7">
              <Clock className="h-3 w-3" />
              Med{" "}
            ...
            <TabsTrigger value="low" className="gap-1 text-xs h-7">
              <CheckCircle2 className="h-3 w-3" />
              Low{" "}
              <Badge variant="outline" className="text-[10px] px-1 py-0 ml-0.5">
                {priorityCounts.low}
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-sm text-foreground">Supplier Concentration</span>
              </div>
                ...
                {supplierConcentration[0]?.percent > 50 && (
                  <div className="p-3 rounded-lg bg-orange-500/10 border border-orange-500/20 max-w-[200px]">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {supplierConcentration[0].percent}% of orders depend on {supplierConcentration[0].name}.
                    </p>
                  </div>
                )}
              </div>
                ...
                <button className="w-full flex items-center justify-between p-3 hover:bg-primary/10 transition-colors rounded-t-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground hidden sm:inline">
                      Items frequently ordered (avg monthly qty)
                    </span>
                  </div>
                  {suggestedOpen ? (
                    <ChevronDown className="h-4 w-4 text-muted-foreground" />
                    ...
                    <Table className="w-full table-fixed">
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-right">Avg Qty/mo</TableHead>
          ...
          <div className="sticky bottom-0 z-20 bg-background/95 backdrop-blur-sm border-t border-border -mx-1 px-1 pt-2 pb-1">
            <div className="flex items-center gap-2 p-2.5 rounded-lg bg-primary/10 border border-primary/20 flex-wrap">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="text-xs text-muted-foreground">
                  Qty <strong className="text-foreground">{selectionTotals.qty.toLocaleString()}</strong>
                </span>
                {selectionTotals.cost > 0 && (
                  <span className="text-xs text-muted-foreground">
                    Est. <strong className="text-foreground">R{selectionTotals.cost.toLocaleString()}</strong>
                  </span>
                )}
                <span className="text-xs text-muted-foreground">
                  {selectionTotals.suppliers} supplier{selectionTotals.suppliers !== 1 ? "s" : ""}
                </span>
                {selectionTotals.urgent > 0 && (
                  <Badge variant="destructive" className="text-[10px] h-4 gap-0.5">
                    <Flame className="h-2.5 w-2.5" />
                    {selectionTotals.urgent} urgent
                  </Badge>
                )}
              </div>
              <div className="flex items-center gap-1.5 ml-auto flex-wrap">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={fillRecommendedForSelected}
                  ...
                  <TooltipContent>
                    <p className="text-xs">
                      Set order qty to the recommended value (incl. safety stock) for all selected items
                    </p>
                  </TooltipContent>
                </Tooltip>
                ...
                <Button variant="outline" size="sm" onClick={handleCopySelectedPOLines} className="h-7 text-xs gap-1">
                  <ClipboardCopy className="h-3 w-3" />
                  PO Lines
                </Button>
                <Button variant="outline" size="sm" onClick={handleCopySupplierEmails} className="h-7 text-xs gap-1">
                  <Mail className="h-3 w-3" />
                  Emails
                </Button>
                ...
                <Button
                  ...
                  className="h-7 text-xs gap-1"
                >
                  {bulkOrdering ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckSquare className="h-3 w-3" />}
                  Mark Ordered
                </Button>
                <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setSelectedSkus(new Set())}>
                  Clear
                </Button>
              </div>
            </div>
          ...
          <>
            ...
            <div className="procurement-table-toolbar flex flex-col xl:flex-row xl:items-center xl:justify-between gap-2 rounded-2xl border border-border/60 bg-card/70 p-2.5">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Button
                  variant={showSupplierChart ? "default" : "outline"}
                  size="sm"
                  onClick={() => setShowSupplierChart(!showSupplierChart)}
                  className="h-7 text-xs gap-1.5"
                >
                  <PieChart className="h-3 w-3" />
                  Chart
                </Button>
                <Button variant="outline" size="sm" onClick={handleCopySupplierEmails} className="h-7 text-xs gap-1.5">
                  <Mail className="h-3 w-3" />
                  Emails
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={handleBatchEmailAllSuppliers}
                  className="h-7 text-xs gap-1.5"
                >
                  <Send className="h-3 w-3" />
                  All Drafts
                </Button>
                <Button variant="outline" size="sm" onClick={selectAllNeedingOrder} className="h-7 text-xs gap-1.5">
                  <CheckSquare className="h-3 w-3" />
                  Select Needing Order
                </Button>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Badge variant="destructive" className="text-xs gap-1 cursor-help">
                        <AlertTriangle className="h-3 w-3" />
                        {coverageGaps.length} no supplier
                      </Badge>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="text-xs">
                        {coverageGaps.length} items need ordering but have no supplier assigned:
                        <br />
                        {coverageGaps
                          .slice(0, 5)
                          .map((r) => r.sku)
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded cursor-help">
                        +{totalSafetyBuffer} safety
                      </span>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="text-xs">
                        Recommended: {totalRecommendedQty} total (incl. {totalSafetyBuffer} safety stock buffer)
                      </p>
                    </TooltipContent>
                  </Tooltip>
                )}
              ...
              <div className="hidden 2xl:flex items-center gap-2 text-[10px] text-muted-foreground/50">
                <span>
                  <kbd className="px-1 py-0.5 rounded bg-muted text-[10px]">Ctrl+F</kbd> Search
                </span>
                <span>
                  <kbd className="px-1 py-0.5 rounded bg-muted text-[10px]">G</kbd> Group
                </span>
                <span>
                  <kbd className="px-1 py-0.5 rounded bg-muted text-[10px]">P</kbd> Density
                </span>
                {pinnedSkus.length > 0 && (
                  <span className="flex items-center gap-0.5">
                    <Pin className="h-2.5 w-2.5" />
                    {pinnedSkus.length} pinned
                  </span>
                )}
                <span>Click row to expand</span>
                <span>
                  <kbd className="px-1 py-0.5 rounded bg-muted text-[10px]">Shift+Click</kbd> Range select
                </span>
              </div>
            </div>
                    ...
                    <TableHeader>
                      <TableRow>
                        <TableHead className="w-8">
                          <Checkbox
                            checked={selectedSkus.size === sortedRows.length && sortedRows.length > 0}
                        ...
                        <SortableHeader field="totalNeeded" className="text-right">
                          Needed
                        </SortableHeader>
                        <SortableHeader field="stockOnHand" className="text-right">
                          Stock
                        </SortableHeader>
                        <SortableHeader field="onPurchaseOrder" className="text-right">
                          On PO
                        </SortableHeader>
                        <TableHead>Coverage</TableHead>
                        <SortableHeader field="toOrder" className="text-right">
                          Order
                        </SortableHeader>
                        <SortableHeader field="stockoutRiskDays" className="text-center">
                          Risk
                        </SortableHeader>
                        <TableHead className="text-center">Trend</TableHead>
                        <SortableHeader field="daysWaiting" className="text-center">
                          Wait
                        </SortableHeader>
                    ...
                    <TableBody>
                                ...
                                <TableCell colSpan={15}>
                                  <div className="flex items-center justify-between gap-2 flex-wrap">
                                    <div className="flex items-center gap-2">
                                      <span
                                        onClick={(e) => {
                                          ...
                                          setSelectedSkus((prev) => {
                                            const next = new Set(prev);
                                            if (allSelected) items.forEach((r) => next.delete(r.sku));
                                            else items.forEach((r) => next.add(r.sku));
                                            return next;
                                          });
                                        }}
                                        className="inline-flex"
                                    ...
                                    <div className="flex items-center gap-3 text-xs">
                                      <span className="text-muted-foreground">
                                        Order: <strong className="text-primary">{supTotalQty.toLocaleString()}</strong>
                                      </span>
                                      {supTotalRec !== supTotalQty && (
                                        <span className="text-muted-foreground">
                                          Rec:{" "}
                                          <strong className="text-foreground">{supTotalRec.toLocaleString()}</strong>
                                        </span>
                                      )}
                                      {supTotalCost > 0 && (
                                        <span className="text-muted-foreground">
                                          Est:{" "}
                                          <strong className="text-foreground">R{supTotalCost.toLocaleString()}</strong>
                                        </span>
                                      )}
                                      <Button
                                        variant="ghost"
                                        ...
                                        onClick={() => generateEmailDraft(supplier)}
                                      >
                                        <Send className="h-3 w-3" />
                                        Email
                                      </Button>
                      ...
                      {sortedRows.length > 0 && (
                        <TableRow className="bg-muted/60 font-semibold border-t-2 border-border">
                          <TableCell colSpan={4} className="text-xs text-muted-foreground uppercase">
                            Totals ({sortedRows.length} SKUs)
                          </TableCell>
                          ...
                          <TableCell>
                            <CoverageBar
                              percent={
                                totals.needed > 0
                                  ? Math.min(100, Math.round(((totals.inStock + totals.onPO) / totals.needed) * 100))
                                  : 100
                              }
                            />
                          </TableCell>
                          <TableCell className="text-right">
                            <Badge variant="destructive" className="font-bold">
                              {totals.toOrder.toLocaleString()}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-center">
                            <span className="text-xs">{totals.stockoutRisk} at risk</span>
                          </TableCell>
                          <TableCell />
                          <TableCell className="text-center">
                            <span className="text-xs">{avgDaysWaiting}d avg</span>
                          </TableCell>
                          <TableCell colSpan={3} />
                        </TableRow>
                      )}
            ...
            <DialogFooter className="gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  toast({ title: "Copied", description: "Email draft copied" });
                }}
              >
              ...
              <Button
                onClick={() => {
                  const s = sortedRows.find((r) => r.supplierName === emailDraftSupplier && r.supplierEmail);
                  window.open(
                    `mailto:${s?.supplierEmail || ""}?subject=${encodeURIComponent(emailDraftSubject || `Purchase Order — ${emailDraftSupplier}`)}&body=${encodeURIComponent(emailDraftBody)}`,
                    "_blank",
                  );
                }}
              >
